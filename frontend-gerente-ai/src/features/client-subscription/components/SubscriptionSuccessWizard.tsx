...
export function SubscriptionSuccessWizard({
  ...
  /*
   * Los mensajes de IA no forman parte de PlanBackend.
   * Se obtienen desde el catálogo centralizado por ID de plan.
   */
  const mensajesIa =
    MENSAJES_IA_POR_PLAN[plan.id] ?? 'Mensajes de IA incluidos';
  ...
  const slides = [
    {
      ...
      description:
        'Automatiza el registro de ventas, gastos y consultas financieras usando lenguaje natural.',
      features: [
        {
          icon: Mic,
          text: 'Registro por notas de voz en WhatsApp',
        },
        {
          icon: Camera,
          text: 'Lectura inteligente de tickets y facturas',
        },
        {
          icon: Zap,
          text: mensajesIa,
    ...
    },
    {
      ...
      description: `Capacidad desbloqueada para administrar hasta ${
        plan.maxSedes || 1
      } sedes independientes con consolidado total.`,
      features: [
        {
          icon: Building2,
          text: `Hasta ${plan.maxSedes || 1} sucursales sincronizadas`,
        },
        {
          icon: Layers,
          text: 'Métricas consolidadas o por sede en 1 clic',
        },
        {
          icon: ShieldCheck,
          text: 'Permisos para administradores y cajeros',
        },
      ],
      highlightPill: `🏢 ${
        plan.maxSedes || 1
    ...
    },
    {
      ...
      description:
        'Reportes automáticos de rentabilidad, productos top y seguimiento en tiempo real de cuentas por cobrar.',
      features: [
        {
          icon: TrendingUp,
          text: 'Cálculo de margen y balance diario',
        },
        {
          icon: DollarSign,
          text: 'Control y recordatorios de cobro a fiados',
        },
        {
          icon: Check,
          text: 'Exportación de informes contables',
        },
      ],
      ...
      <div className="space-y-3 text-center">
        <div className="flex items-center justify-between">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[11px] font-bold text-emerald-600 dark:text-emerald-400">
            ...
            Pago Aprobado • $
            {PRECIO_FORMATTER.format(montoTotal)} COP
          </div>
